...
/* MODIFIED by Xinze Zhao and Zimu Liu for the purpose of assignment.
 *
 *
 */
 ...
 */
#define    EXT2_S_IFLNK  0xA000    /* symbolic link */ //7
#define    EXT2_S_IFREG  0x8000    /* regular file */  //1
#define    EXT2_S_IFDIR  0x4000    /* directory */     //2
...
/* Maximum length of a file/dir name. */
#define MAX_NAME     255

/* Size of one byte in bits. */
#define BYTE_IN_BITS 8

/* Global variables for various base pointers. */
unsigned char *disk;
struct ext2_inode *inode_table;
struct ext2_group_desc *gd;
struct ext2_super_block *sb;
unsigned char *block_bitmap_base;
unsigned char *inode_bitmap_base;
...
typedef struct node {
    ...
    char name[MAX_NAME];
    /* Point to the child directory. */
    struct node *next;
} path_node;

/*
 * Linked list representation of an absolute path.
 */
typedef struct {
    path_node *head;
    path_node *tail; 
...
/* utils.c */
extern void ext2_base_init(int);				  /* Initialize various base pointers. */

extern void free_path_list(path_node_list *);     /* Free the dynamically allocated memory. */

extern int restore(int, char *);                  /* Restore the specified file. */

extern int checker(int);

extern int do_the_checkings(struct ext2_dir_entry *);

extern int check_counter(int);                    /* Check the free count in gd and bg. */

extern int check_mode(struct ext2_dir_entry *);

extern int check_bitmap(struct ext2_dir_entry *, int);

extern int check_dtime(struct ext2_dir_entry *);

/* path.c */
extern void split_path(char *, path_node_list *); /* Split the absolute path and store the paths into path_node_list. */

extern int dfs_visit(int, int);					  /* Recursive routine to add the block numbers using depth-first-search. */

extern void dfs(unsigned int *);				  /* Add the block numbers of i_block to block_list. */

extern void clear_block_list(void);				  /* Clear the global variables updated by the routine dfs. */

extern int get_block_nums(int *);
/* Check whether a dir with specified name exists under the directory with
 * the given inode_num.
 *
 * Return the inode number of the matched dir on success, -1 on fail. */
extern int find_dir(int, char *, int);

/* Get the inode_number of the parent dir of specified directory. */
extern int get_inode_num(int, path_node *, path_node_list *, int);


/* alloc.c */
extern void allocate_dir(unsigned int, char *, int, unsigned int, int); /* Main dirver of alloc.c */

extern void dealloc_file(unsigned int, unsigned int, char *);           /* Delete a file on the disk. */

extern void alloc_hardlink(int, int, char *);                           /* Allocate a hardlink. */

extern void alloc_symblink(char *, int, char *);                        /* Allocate a symbolic link. */

extern void alloc_block(int, int, char*);/* Allocate block(s) for a directory or a regular file. */

extern void alloc_inode(int);			 /* Allocate an inode for a directory or a regular file. */

extern void dealloc_dir_block(int, char *);     /* Update the directroy entry of removed file/dir. */

extern int set_bitmap(int);              /* Find the first one available from the corresponding bitmap.*/

extern void convert_to_bin(int *, int);  /* Convert decimal number to binary and store these figures. */

extern int convert_to_dec(int *);		 /* Convert the binary to decimal. */

extern int set_bit(int *);				 /* Set the first available bit. */

extern void set_specific_bit(int, int, int);/* Set a specific bit. */

extern int check_bit(int, int);          /* Check whether the specified bit is set or not. */

extern void init_file_block(int);        /* Initialize the file block i.e. write all the data to the disk. */

extern void init_dir_block(void);		 /* Initialize a directory block. */

extern void init_symb_block(char *);     /* Initialize a block for a symbolic link. */

extern void update_inode_block(void);    /* Update the inode after allocating block(s) for a dir/file. */

extern void update_parent_dir_block(int);/* Update the parent directory block. */

extern int recover_entry(struct ext2_dir_entry *, char *, int);

extern void update_parent_inode(int);    /* Update the parent inode after creating a new dir/file. */

extern void append_to_block(int, int*, int);
                                         /* Add an array of block_num's to the specified i_block. */

/* Bonus. */
extern int check_dir_entry(struct ext2_dir_entry *, int);
